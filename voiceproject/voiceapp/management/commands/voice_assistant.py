...
class AudioLoop:
    ...
    async def push_client_audio(self, pcm_bytes: bytes, mime_type: str = f"audio/pcm;rate={SEND_RATE}"):
        """Called by your WebSocket consumer when the browser sends PCM chunks."""
        if not pcm_bytes:
            return
    ...
    async def run(self):
        try:
            # Optional conversation bootstrapping
...
# ---- Keep a no-op management command so imports don't break if you ever call it ----
